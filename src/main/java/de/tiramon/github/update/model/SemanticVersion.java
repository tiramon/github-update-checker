...
/**
 * Implementation for the comparison between semantic versions.
 */
public final class SemanticVersion implements Comparable<SemanticVersion> {
	 ...
	 * @param preRelease pre release identifiers. Must not be null, all parts must match "[0-9A-Za-z-]+".
	 * @param buildMeta  build meta identifiers. Must not be null, all parts must match "[0-9A-Za-z-]+".
	 */
	...
	/**
	 * Convenience constructor for creating a Version object from the "Implementation-Version:" property of the Manifest file.
	 * 
	...
	/**
	 * When major, minor, patch are equal this method is used to compare the preReleaseTags. The method compares the amount of pre release tags and compares the content of the pre release tags.
	 * 
	 * @param v the object to be compared
	 * @return a negative integer, zero, or a positive integer as this object is less than, equal to, or greater than the specified object.
	 */
	private int comparePreRelease(SemanticVersion v) {
		int result = 0;
	...
	private boolean stateRelease(int index) {
		int pos = index;
		while ((pos < input.length) && ((input[pos] >= '0' && input[pos] <= '9') || (input[pos] >= 'a' && input[pos] <= 'z') || (input[pos] >= 'A' && input[pos] <= 'Z') || input[pos] == '-')) {
			pos++; // match [0..9a-zA-Z-]+
		}
		if (pos == index) { // Empty String -> Error
			errPos = index;
